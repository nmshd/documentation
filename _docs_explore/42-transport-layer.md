...
The transport layer is located between the backbone layer and the consumption layer. Thus it acts as the interface between the trusted environment (own device/network) and the untrusted environment (Backbone). It is usually hosted as a REST API by the [Connector]({{ site.baseurl }}{% link _docs_explore/52-connector.md %}) or programmatically accessed by the [App]({{ site.baseurl }}{% link _docs_explore/50-app.md %}). The [Runtime]({{ site.baseurl }}{% link _docs_explore/60-runtime.md %}) is the primary technical building block, containing the transport library, the content library and the crypto library.
