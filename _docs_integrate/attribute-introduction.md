...
Moreover, the prefixes `urn:`, `language:` and `mimetype:` are supported, whereby `language:` must be followed by a valid ISO 639 language code and `mimetype:` by a valid MIME type matching the pattern `^[a-z-*]+/[a-z-*]+$`.
