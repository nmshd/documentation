...
Of course, the `shareInfo` property of the own shared RelationshipAttribute is set. Within the `shareInfo.peer` property, the Address of the peer to which the RelationshipAttribute `owner` has established the Relationship is specified.
...
Although the peer is not its `owner`, the underlying RelationshipAttribute of a peer shared RelationshipAttribute can be shared with peers who are not involved in the Relationship in which the RelationshipAttribute exists if its `confidentiality` is not `"private"`.
The sharing of the RelationshipAttribute by the peer with another peer is technically documented by the creation of a so-called third party owned RelationshipAttribute.

#### Third party owned RelationshipAttributes

[RelationshipAttributes]({% link _docs_integrate/data-model-overview.md %}#relationshipattribute) can be shared with peers who are not involved in the Relationship in which the RelationshipAttribute exists.
If the `owner` of the RelationshipAttribute is not the one who shares it, but the peer who has established the Relationship with the `owner` in which the RelationshipAttribute exists, this corresponds to sharing an underlying RelationshipAttribute of a peer shared RelationshipAttribute.
Such sharing leads to the creation of two certain [LocalAttributes]({% link _docs_integrate/data-model-overview.md %}#localattribute), the so-called third party owned RelationshipAttributes.
...
The latter is the case until the peer shared RelationshipAttribute used as the source has not been [deleted]({% link _docs_integrate/delete-attributes.md %}#delete-peer-shared-attributes).
