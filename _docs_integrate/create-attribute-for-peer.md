...
The following table provides an overview of the possible kinds of Attributes that the Sender can create for the Recipient using the CreateAttributeRequestItem. It must be taken into account whether the [Attribute]({% link _docs_integrate/data-model-overview.md %}#attributes) is an IdentityAttribute or a RelationshipAttribute and which Identity is its `owner`. Note that you can only explicitly specify the Recipient as the `owner` of the Attribute for [Requests that are sent via a Message]({% link _docs_integrate/create-attribute-for-peer.md %}#request-via-message). This is because when a Request is sent via a Message, the Identity that receives the Request is always known in advance. For [Requests that are sent via a RelationshipTemplate]({% link _docs_integrate/create-attribute-for-peer.md %}#request-via-relationshiptemplate), an empty string must be specified as the `owner` instead, as you do not know the Identity that will load your RelationshipTemplate beforehand. Specifying an empty string for the `owner` of the Attribute is equivalent to requesting the creation of an Attribute with the Recipient as its `owner`.

| Type                  | Owner       | Possible? | Automation      | Remarks, reasons and examples                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| --------------------- | ----------- | :-------: | --------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| IdentityAttribute     | Sender      |     ✗     | `N/A`           | Use the [ShareAttributeRequestItem]({% link _docs_integrate/data-model-overview.md %}#shareattributerequestitem) instead. For more details, refer to the documentation of the [Share own Attribute to peer]({% link _docs_integrate/share-own-attribute-to-peer.md %}) scenario.                                                                                                                                                                                                                                                                                                                                                                                                                            |
| IdentityAttribute     | Recipient   |     ✓     | `USER_DECISION` | The Sender can only explicitly specify the Recipient as the `owner` of the IdentityAttribute in a [Request that is sent via a Message]({% link _docs_integrate/create-attribute-for-peer.md %}#request-via-message). An empty string must be specified instead if a [Request that is sent via a RelationshipTemplate]({% link _docs_integrate/create-attribute-for-peer.md %}#request-via-relationshiptemplate) is used.<br>Example: A university sends a student their certificate. (Proposing a corresponding IdentityAttribute would be inappropriate in this case, because the student should not be able to return their own value.)                                                                   |
| IdentityAttribute     | Third party |     ✗     | `N/A`           | It makes no sense to create an IdentityAttribute for the Recipient that is owned by a third party.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| RelationshipAttribute | Sender      |     ✓     | `AUTO_ACCEPT`   | Example: A company sends a new customer their customer number.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| RelationshipAttribute | Recipient   |     ✓     | `USER_DECISION` | With this combination, the **Sender asks the Recipient for the one-time permission** to write a RelationshipAttribute once **and** the **Sender defined a value** which can either be accepted and stored, or rejected. Thus, the Recipient cannot change the value by itself. The Sender can only explicitly specify the Recipient as the `owner` of the RelationshipAttribute in a [Request that is sent via a Message]({% link _docs_integrate/create-attribute-for-peer.md %}#request-via-message). An empty string must be specified instead if a [Request that is sent via a RelationshipTemplate]({% link _docs_integrate/create-attribute-for-peer.md %}#request-via-relationshiptemplate) is used. |
| RelationshipAttribute | Third party |     ✗     | `N/A`           | A CreateAttributeRequestItem can only be used to request the creation of RelationshipAttributes that exist in the context of the Relationship between the Sender and the Recipient. For this reason, creating a RelationshipAttribute for the Recipient that is owned by a third party is not possible.                                                                                                                                                                                                                                                                                                                                                                                                     |

### Example of creating an IdentityAttribute

We assume that the Integrator of the Sender wants to create an IdentityAttribute of type [EMailAddress]({% link _docs_integrate/attribute-values.md %}#emailaddress) that is owned by the Recipient. To request the creation of this IdentityAttribute for the Recipient, the Sender needs to insert it into the `attribute` property of the [CreateAttributeRequestItem]({% link _docs_integrate/data-model-overview.md %}#createattributerequestitem) contained within the `items` property of the [Request]({% link _docs_integrate/data-model-overview.md %}#request) for creating Attributes. As the IdentityAttribute should be owned by the Recipient, an empty string is specified for its `owner` property. If the Sender sends the corresponding [Request via a Message]({% link _docs_integrate/create-attribute-for-peer.md %}#request-via-message), the Address of the Recipient could alternatively be specified explicitly. In our example, we have chosen to set the value of the `mustBeAccepted` property of the CreateAttributeRequestItem to `true`. Please note that the `<...>` notation is used as a placeholder for the actual data as usual.
...
- If the [Request was sent via a RelationshipTemplate]({% link _docs_integrate/create-attribute-for-peer.md %}#request-via-relationshiptemplate): Specify `<ID of RelationshipTemplate>` as the value for the `source.reference` query parameter.
- If the [Request was sent via a Message]({% link _docs_integrate/create-attribute-for-peer.md %}#request-via-message): Specify `<ID of Request>` as the value for the `id` query parameter.
