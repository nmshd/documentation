...
The following table provides an overview of the possible kinds of Attributes that the Sender can create for the Recipient using the CreateAttributeRequestItem. It must be taken into account whether the [Attribute]({% link _docs_integrate/data-model-overview.md %}#attributes) is an IdentityAttribute or a RelationshipAttribute and which Identity is its `owner`. Note that you can only explicitly specify the Recipient as the `owner` of the Attribute for [Requests that are sent via a Message]({% link _docs_integrate/create-attribute-for-peer.md %}#request-via-message). This is because when a Request is sent via a Message, the Identity that receives the Request is always known in advance. For [Requests that are sent via a RelationshipTemplate]({% link _docs_integrate/create-attribute-for-peer.md %}#request-via-relationshiptemplate), an empty string must be specified as the `owner` instead, as you do not know the Identity that will load your RelationshipTemplate beforehand. Specifying an empty string for the `owner` of the Attribute is equivalent to requesting the creation of an Attribute with the Recipient as the `owner`.

| Attribute Type | Attribute Owner | Possible? | Automation      | Examples/Reason                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| -------------- | --------------- | :-------: | --------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Identity       | Sender          |     ✗     | `N/A`           | Use the [ShareAttributeRequestItem]({% link _docs_integrate/data-model-overview.md %}#shareattributerequestitem) instead. For more details, refer to the documentation of the [Share own Attribute to peer]({% link _docs_integrate/share-own-attribute-to-peer.md %}) scenario.                                                                                                                                                                                                                |
| Identity       | Recipient       |     ✓     | `USER_DECISION` | A university sends a student their certificate. (Proposing a corresponding IdentityAttribute would be inappropriate in this case, because the student should not be able to return their own value.) You can only explicitly specify the Recipient as the `owner` of the IdentityAttribute in a [Request that is sent via a Message]({% link _docs_integrate/create-attribute-for-peer.md %}#request-via-message).                                                                              |
| Identity       | Third Party     |     ✗     | `N/A`           | It makes no sense to create an IdentityAttribute for the Recipient that is owned by a third party.                                                                                                                                                                                                                                                                                                                                                                                              |
| Identity       | `<empty>`       |     ✓     | `USER_DECISION` | Specifying an empty string for the `owner` of the IdentityAttribute is equivalent to requesting the creation of an IdentityAttribute with the Recipient as the `owner`. Offering this option is necessary when using a [Request that is sent via a RelationshipTemplate]({% link _docs_integrate/create-attribute-for-peer.md %}#request-via-relationshiptemplate).                                                                                                                             |
| Relationship   | Sender          |     ✓     | `AUTO_ACCEPT`   | A company sends a new customer their customer number.                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| Relationship   | Recipient       |     ✓     | `USER_DECISION` | With this combination the **Sender asks the Recipient for the one-time permission** to write a RelationshipAttribute once **and** the **Sender defined a value** which can either be accepted and stored, or rejected. Thus, the Recipient cannot change the value by itself. You can only explicitly specify the Recipient as the `owner` of the RelationshipAttribute in a [Request that is sent via a Message]({% link _docs_integrate/create-attribute-for-peer.md %}#request-via-message). |
| Relationship   | Third Party     |     ✗     | `N/A`           | It makes no sense to create a RelationshipAttribute for the Recipient that is owned by a third party.                                                                                                                                                                                                                                                                                                                                                                                           |
| Relationship   | `<empty>`       |     ✓     | `USER_DECISION` | Specifying an empty string for the `owner` of the RelationshipAttribute is equivalent to requesting the creation of a RelationshipAttribute with the Recipient as the `owner`. Offering this option is necessary when using a [Request that is sent via a RelationshipTemplate]({% link _docs_integrate/create-attribute-for-peer.md %}#request-via-relationshiptemplate).                                                                                                                      |
