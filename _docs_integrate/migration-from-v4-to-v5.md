...
- The error code `error.consumption.attributes.invalidPropertyValue` was removed and substituted by [`error.consumption.attributes.wrongOwnerOfAttribute`]({% link _docs_integrate/error-codes.md %}#error.consumption.attributes.wrongOwnerOfAttribute), which is a more specific error code.
