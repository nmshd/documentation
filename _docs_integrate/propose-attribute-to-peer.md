...
| RelationshipAttribute | Third party |     ✗     | `N/A`           | A ProposeAttributeRequestItem can only be used to propose the creation of RelationshipAttributes that exist in the context of a Relationship between the Sender and the Recipient. For this reason, proposing a RelationshipAttribute to the Recipient that is owned by a third party is not possible.                                                        |
