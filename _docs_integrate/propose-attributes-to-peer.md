...
If the Recipient accepted with a new Attribute or an existing one that wasn't shared with the peer before, an appropriate AcceptResponseItem of type [ProposeAttributeAcceptResponseItem]({% link _docs_integrate/data-model-overview.md %}#proposeattributeacceptresponseitem) is generated.
