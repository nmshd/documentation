...
| RelationshipAttribute that exists in the context of a Relationship between the Sender and a third party | Sender      | ✓ or ✗ -<br>depending on `confidentiality` | `AUTO_ACCEPT`   | The Sender can share RelationshipAttributes of a Relationship between itself and a third party that are owned by itself with the Recipient if their `confidentiality` is not `"private"`.<br>Example: A customer shares their customer ID from one company with another company, whereby the customer is the `owner` of the corresponding RelationshipAttribute.                                                                      |
