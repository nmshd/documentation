...
properties:
  ...
  - created_at:
  - changed_at:
  - api_route_regex: GET /api/core/v1/Attributes/Peer/Shared/Identity
  - published: default
...
- `query` allows to specify the conditions for the returned LocalAttributes. In detail, the following keys may be used:
  ...
  - The fields of the `deletionInfo` describe the [ReceivedAttributeDeletionInfo]({% link _docs_integrate/data-model-overview.md %}#receivedattributedeletioninfo) of a shared LocalAttribute.
