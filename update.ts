...
async function createScenarioText(scenarioObject: DynamicUseCase): Promise<string> {
    var text = "---\n";
        ...
        if (
            ...
            if (Object.prototype.hasOwnProperty.call(scenarioObject, key)) {
                ...
                if (value == null) {
                    ...
                    text += "  - " + `${key.toLowerCase()}: ${typeof value === "string" ? value.replaceAll("\n", " ") : value}\n`;
                }
            }
    }
    ...
    var path = "/workspaces/documentation/_includes/scenarios/" + scenarioObject.Component + "/" + scenarioObject.Link + ".md";

    try {
        const fileContent = await dateiLesen(path);
        text += fileContent; // Append the content of the file to the text
    } catch (error) {
        console.error("Error reading file:", error);
    }

    console.log(text); // Display the updated text
    return text;
}

function dateiLesen(dateiPfad: string): Promise<string> {
...
async function createUseCaseText(useCaseObject: DynamicUseCase): Promise<string> {
    var text = "---\n";
    ...
    text += "---\n\n";
    var path = "/workspaces/documentation/_includes/use-cases/" + useCaseObject.Link + ".md";
...
async function main() {
    try {
        ...
        if (scenariosObject && scenariosObject.length > 0) {
            scenariosObject.forEach(async (scenarioObject) => {
                if (scenarioObject.ID != null) {
                    var scenarioText = createScenarioText(scenarioObject);
                    writeTextToFile(scenarioObject, await scenarioText);
                }
            });
        } else {
    ...
    try {
        ...
        if (useCasesObject && useCasesObject.length > 0) {
            useCasesObject.forEach(async (useCaseObject) => {
                if (useCaseObject.ID != null) {
                    var useCaseText = createUseCaseText(useCaseObject);
                    writeTextToFile(useCaseObject, await useCaseText);
                }
            });
        } else {
            console.log("The array is empty or undefined.");
